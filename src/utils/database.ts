...
const menuItemsColRef = (stallId: string) =>
  collection(firestoreDb!, 'users', stallId, 'menu_items');
const menuItemDocRef = (stallId: string, itemId: string) =>
  doc(firestoreDb!, 'users', stallId, 'menu_items', itemId);
const userOrdersRef = (uid: string) => collection(firestoreDb!, 'users', uid, 'orders');
const orderDocRef = (uid: string, orderId: string) =>
  doc(firestoreDb!, 'users', uid, 'orders', orderId);
const walletDocRef = (uid: string) => doc(firestoreDb!, 'users', uid, 'wallet', 'main');
const userDocRef = (uid: string) => doc(firestoreDb!, 'users', uid);

const localOrdersForUid = (uid: string): Order[] =>
  JSON.parse(localStorage.getItem(ordersKey(uid)) || '[]');
...
export const db = {
  initialize: async () => {
    ...
    if (!localStorage.getItem(ordersKey(DEFAULT_CUSTOMER_UID))) {
      localStorage.setItem(
        ordersKey(DEFAULT_CUSTOMER_UID),
        JSON.stringify(initialOrdersFor(DEFAULT_CUSTOMER_UID))
      );
    }
      ...
      try {
        ...
        for (const stall of currentStalls) {
          await db.ensureUserProfile(
            stall.id,
            'foodkiosk',
            `${stall.ownerUsername}@biteflow.app`,
            stall.name
          );
        }
  ...
  createStall: async (
    input: Omit<Stall, 'ownerPasswordEnc'> & { ownerPasswordPlain: string }
  ): Promise<Stall> => {
  ...
  updateStall: async (stall: Stall): Promise<void> => {
    if (firestoreDb) {
      try {
        ...
        await updateDoc(userDocRef(stall.id), {
          displayName: stall.name,
          email: `${stall.ownerUsername}@biteflow.app`,
        });
        return;
      } catch (e) {
  ...
  deleteStall: async (stallId: string): Promise<void> => {
    ...
    localStorage.setItem(
      MENU_ITEMS_KEY,
      JSON.stringify(items.filter((item: any) => item.stallId !== stallId))
    );
  },

  // Decrypt a stall's password for display (admin quick-login list) or login verification.
  getStallPlainPassword: async (stall: Stall): Promise<string> =>
    decryptText(stall.ownerPasswordEnc),
  ...
  deleteMenuItem: async (stallId: string, id: string): Promise<void> => {
    ...
    localStorage.setItem(
      MENU_ITEMS_KEY,
      JSON.stringify(items.filter((item: any) => item.id !== id))
    );
  },
  ...
  getWallet: async (uid: string): Promise<UserWallet> => {
    ...
    const wallet: UserWallet =
      uid === DEFAULT_CUSTOMER_UID ? defaultWallet : { uid, balance: 0, transactions: [] };
