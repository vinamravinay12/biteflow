...
export const StallDashboard: React.FC<StallDashboardProps> = ({ stall, onLogout }) => {
  ...
  const imagePresets = [
    ...
    {
      label: 'Boba Tea',
      url: 'https://images.unsplash.com/photo-1541658016709-82535e94bc69?w=500',
    },
  ...
  const handleUpdateStatus = async (orderId: string, newStatus: OrderStatus, reason?: string) => {
    ...
    const success = await db.updateKioskOrderStatus(
      order.customerUid,
      orderId,
      stall.id,
      newStatus,
    ...
    if (success) {
      ...
      if (newStatus === 'cancelled') {
        ...
        await db.refundWalletFunds(order.customerUid, order.subtotal, refundNote);
      }
      await loadData();
    }
  };
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
          ...
          <div>
            <h1
              ...
              style={{ fontSize: '2.25rem', fontWeight: 800, margin: 0, color: 'white' }}
            >
              {stall.name}
            </h1>
            <p
            ...
            <div
              style={{ display: 'flex', gap: '1rem', marginTop: '0.75rem', alignItems: 'center' }}
            >
              <span
        ...
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
          ...
          <button
            onClick={loadData}
            className="btn btn-secondary"
            title={KIOSK_TRANSLATIONS[language].refreshData}
            aria-label={KIOSK_TRANSLATIONS[language].refreshData}
          >
      ...
      >
        <button
          ...
          style={{
            ...
            cursor: 'pointer',
            borderBottom:
              activeTab === 'orders' ? '3px solid var(--accent-cyan)' : '3px solid transparent',
            display: 'flex',
            alignItems: 'center',
            gap: '0.5rem',
        ...
        <button
          ...
          style={{
            ...
            cursor: 'pointer',
            borderBottom:
              activeTab === 'menu' ? '3px solid var(--accent-cyan)' : '3px solid transparent',
            display: 'flex',
            alignItems: 'center',
            gap: '0.5rem',
            transition: 'all 0.15s ease',
        ...
        <button
          ...
          style={{
            ...
            cursor: 'pointer',
            borderBottom:
              activeTab === 'analytics' ? '3px solid var(--accent-cyan)' : '3px solid transparent',
            display: 'flex',
            alignItems: 'center',
            gap: '0.5rem',
            transition: 'all 0.15s ease',
            ...
            {(['all', 'pending', 'preparing', 'ready', 'completed', 'cancelled'] as const).map(
              (f) => {
                ...
                return (
                  ...
                  >
                    {KIOSK_TRANSLATIONS[language][
                      'status' + f.charAt(0).toUpperCase() + f.slice(1)
                    ] || f}
                    <span
                      style={{
                        opacity: 0.7,
          ...
          {filteredOrders.length === 0 ? (
            ...
            >
              ...
              <h3 style={{ fontSize: '1.25rem', marginBottom: '0.25rem' }}>
                {KIOSK_TRANSLATIONS[language].noOrders}
              </h3>
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
                {orderFilter === 'all'
                  ? DASHBOARD_LOCALES[language].emptyQueueAll
                  : DASHBOARD_LOCALES[language].emptyQueueFilter(orderFilter)}
              </p>
            </div>
          ) : (
                  ...
                  >
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                          ...
                          className={`badge ${
                            order.status === 'pending'
                              ...
                              : order.status === 'preparing'
                                ? 'badge-info'
                                : order.status === 'ready'
                                  ? 'badge-success' // customized coloring
                                  : order.status === 'completed'
                                    ? 'badge-success'
                                    : 'badge-danger'
                          }`}
                        >
                          {order.status === 'ready'
                            ? KIOSK_TRANSLATIONS[language].statusReady
                            : order.status === 'pending'
                              ? KIOSK_TRANSLATIONS[language].statusPending
                              : order.status === 'preparing'
                                ? KIOSK_TRANSLATIONS[language].statusPreparing
                                : order.status === 'completed'
                                  ? KIOSK_TRANSLATIONS[language].statusCompleted
                                  : KIOSK_TRANSLATIONS[language].statusCancelled}
                        </span>
                      </div>
                      <p
                        style={{
                      ...
                      >
                        {DASHBOARD_LOCALES[language].placedOn}{' '}
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                        {KIOSK_TRANSLATIONS[language].customerLabel}
                      </span>
                      <p
                        style={{
                          fontWeight: 600,
                  ...
                  <div style={{ marginBottom: '1.25rem' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                      {order.items.map((item, idx) => (
                        ...
                        >
                          <div
                            style={{ display: 'flex', gap: '0.5rem', color: 'var(--text-primary)' }}
                          >
                    ...
                    {order.notes && (
                      ...
                      >
                        <AlertCircle size={14} />
                        <span>
                          <strong>{KIOSK_TRANSLATIONS[language].orderNotes}:</strong> {order.notes}
                        </span>
                      </div>
                    )}
                    {order.seatNumber && (
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          <span>🏟️</span>
                          <span>
                            <strong>{DASHBOARD_LOCALES[language].stadiumDelivery}:</strong>
                          </span>
                        </div>
                        <div
                          style={{
                        ...
                        >
                          <div>
                            {DASHBOARD_LOCALES[language].match}:{' '}
                            <strong style={{ color: 'white' }}>{order.matchName}</strong>
                          </div>
                          <div>
                            {DASHBOARD_LOCALES[language].location}:{' '}
                            <strong style={{ color: 'white' }}>
                              {order.stand} ({KIOSK_TRANSLATIONS[language].seatLabel}{' '}
                              {order.seatNumber})
                            </strong>
                          </div>
                        </div>
                  ...
                  >
                    ...
                    <div
                      style={{
                        display: 'flex',
                        gap: '0.5rem',
                        width: decliningOrderId === order.orderId ? '100%' : 'auto',
                              ...
                              {[
                                ...
                                <button
                                  ...
                                  style={{
                                    ...
                                    color:
                                      declineReason === reason ? 'white' : 'var(--text-secondary)',
                                    border:
                                      '1px solid ' +
                                      (declineReason === reason
                                        ? 'var(--accent-red)'
                                ...
                                >
                                  {reason === 'Finished / Out of Stock'
                                    ? DASHBOARD_LOCALES[language].reasonStock
                                    : reason === 'Not Available'
                                      ? DASHBOARD_LOCALES[language].reasonNotAvailable
                                      : reason === 'Kiosk Too Busy'
                                        ? DASHBOARD_LOCALES[language].reasonBusy
                                        : DASHBOARD_LOCALES[language].reasonTechnical}
                                </button>
                              ))}
                            </div>
                            <input
        ...
        <div>
          ...
          >
            <div>
              <h2 className="font-display" style={{ fontSize: '1.5rem', fontWeight: 700 }}>
                {KIOSK_TRANSLATIONS[language].menuOfferings}
              </h2>
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
                {KIOSK_TRANSLATIONS[language].menuOfferingsDesc}
              </p>
            </div>
            <button
          ...
          {menuItems.length === 0 ? (
            ...
            >
              ...
              />
              <h3 style={{ fontSize: '1.25rem', marginBottom: '0.25rem' }}>
                {KIOSK_TRANSLATIONS[language].menuEmpty}
              </h3>
              <p
                style={{
                  color: 'var(--text-secondary)',
                  ...
                  >
                    ...
                    <div>
                      ...
                      >
                        <Clock size={12} />
                        <span>
                          {KIOSK_TRANSLATIONS[language].prepTimeLabel}
                          {item.prepTime} {KIOSK_TRANSLATIONS[language].minutesSuffix}
                        </span>
                      </div>
        ...
        <div>
          ...
          >
            ...
            >
              ...
              <div>
                <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                  {KIOSK_TRANSLATIONS[language].revenue}
                </span>
                <h3
                  className="font-display"
            ...
            >
              ...
              <div>
                <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                  {KIOSK_TRANSLATIONS[language].totalOrders}
                </span>
                <h3
                  className="font-display"
                  style={{ fontSize: '1.5rem', fontWeight: 700, color: 'white' }}
            ...
            >
              ...
              <div>
                <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                  {KIOSK_TRANSLATIONS[language].avgOrderValue}
                </span>
                <h3
                  className="font-display"
                  style={{ fontSize: '1.5rem', fontWeight: 700, color: 'white' }}
            ...
            >
              ...
              <div>
                <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                  {KIOSK_TRANSLATIONS[language].cancelledOrders}
                </span>
                <h3
                  className="font-display"
                  style={{ fontSize: '1.5rem', fontWeight: 700, color: 'white' }}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1.5rem' }}>
            ...
            <div className="glass-panel" style={{ padding: '2rem' }}>
              ...
              >
                ...
                {KIOSK_TRANSLATIONS[language].hourlyFrequency}
              </h3>
              <div
                style={{
                  height: '220px',
            ...
            <div className="glass-panel" style={{ padding: '2rem' }}>
              ...
              {menuItems.length === 0 ? (
                <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', fontStyle: 'italic' }}>
                  {KIOSK_TRANSLATIONS[language].noMenuItems}
                </p>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  {menuItems.slice(0, 4).map((item, index) => {
                    // Generate mock sales count
                    const mockSalesCount = Math.floor(
                      (4.9 - index * 0.4) * (stats.completedOrdersCount + 3)
                    );
                    return (
                      <div
                        key={item.id}
        ...
        >
          ...
          >
            ...
            >
              ...
              <div>
                ...
                {imageSource === 'url' && (
                  <div>
                    ...
                    >
                      💡 <strong>{KIOSK_TRANSLATIONS[language].unsplashTip}</strong>{' '}
                      {KIOSK_TRANSLATIONS[language].unsplashTipBody}
                    </p>

                    {/* Warning if they paste a webpage URL */}
                    {customImageUrl &&
                      customImageUrl.includes('unsplash.com/photos/') &&
                      !customImageUrl.includes('images.unsplash.com/') && (
                        ...
                        >
                          ⚠️ <strong>{KIOSK_TRANSLATIONS[language].unsplashWarning}</strong>{' '}
                          {KIOSK_TRANSLATIONS[language].unsplashWarningBody}
                        </p>
                      )}
                    ...
                    {customImageUrl && (
                      ...
                      >
                        ...
                        <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                          {KIOSK_TRANSLATIONS[language].imagePreview}
                        </span>
                      </div>
                    )}
                  </div>
                    ...
                    {uploadedImage && uploadedImage.startsWith('data:') && (
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                          ...
                          >
                            {KIOSK_TRANSLATIONS[language].imageLoaded}
                          </span>
                          <button
                            type="button"
              ...
              {formError && (
                <p style={{ color: 'var(--accent-red)', fontSize: '0.8rem' }}>⚠️ {formError}</p>
              )}
        ...
        >
          ...
          >
            ...
            >
              <div>
                <label
                  htmlFor="merchant-current-password"
                  style={{
                    display: 'block',
                    marginBottom: '0.4rem',
                    fontSize: '0.85rem',
              ...
              {passError && (
                <p style={{ color: 'var(--accent-red)', fontSize: '0.8rem' }}>⚠️ {passError}</p>
              )}
